...
DEFAULT_PRODUCTS = [
    ...
    # TROCKENWAREN 🌾🍝
    (98, 'Reis', 'trockenwaren', '🍚', 'kg'),
    (99, 'Buchweizen', 'trockenwaren', '🌾', 'kg'),
    (100, 'Haferflocken', 'trockenwaren', '🥣', 'kg'),
    (101, 'Hirse', 'trockenwaren', '🌾', 'kg'),
    (102, 'Linsen', 'trockenwaren', '🫘', 'kg'),
    (103, 'Spaghetti', 'trockenwaren', '🍝', 'kg'),
    (104, 'Penne', 'trockenwaren', '🍝', 'kg'),
    (105, 'Fusilli', 'trockenwaren', '🍝', 'kg'),
    (106, 'Lasagneblätter', 'trockenwaren', '🍝', 'pcs'),
    (107, 'Nudeln', 'trockenwaren', '🍜', 'kg'),

    # GEWÜRZE 🧂
    (108, 'Salz', 'gewuerze', '🧂', 'pcs'),
    (109, 'Schwarzer Pfeffer', 'gewuerze', '🌶️', 'pcs'),
    (110, 'Paprikapulver', 'gewuerze', '🌶️', 'pcs'),
    (111, 'Oregano', 'gewuerze', '🌿', 'pcs'),
    (112, 'Curry', 'gewuerze', '🍛', 'pcs'),
]
...
def init_db():
    ...
    cursor.execute("UPDATE products SET category = 'trockenwaren' WHERE category IN ('grains', 'pasta')")

    cursor.executemany(
        "INSERT OR IGNORE INTO products (id, name, category, emoji, unit_type) VALUES (?, ?, ?, ?, ?)",
        DEFAULT_PRODUCTS
    )
    conn.commit()
